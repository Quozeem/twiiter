<?php
 ...
 require "config.php";
 class insertname extends db_connect{
 public function saveRecords($name,$number,$dob,$day,$month)
 {
  // method of object
  
   $select=mysqli_query($this->dbh,"INSERT INTO twits (name,email,dob,day,month)
        VALUES ('$name','$number','$dob','$day','$month')");
 ...
 elseif(empty($number)){
  ...
  while($row=mysqli_fetch_assoc($res)){
   ...
   include_once "config.php";
   $inser=new insertname();
  $sql=$inser->saveRecords($name,$number,$dob,$day,$month);
  if($sql)
  {
...
<a href="signup.php" class="btn">use phone instead</a>
